...
describe("TerminalProcess", () => {
	...
	describe("run", () => {
		...
		describe("streamAvailable timeout", () => {
			...
			it("emits no_shell_integration with commandSubmitted=true when streamAvailable times out", async () => {
				...
				let noSiDetails: { message: string; commandSubmitted: boolean } | undefined
				siProcess.once("no_shell_integration", (details) => {
					noSiDetails = details
				})
				...
				// no_shell_integration fires before completed because the stream
				// timeout emit triggers the constructor handler.
				expect(noSiDetails).toBeDefined()
